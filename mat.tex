...
\maketitle
Since I have already studied the basics of the necessary mathematical tools, for now I want to prioritize physical understanding. Thus, I will deepen my understanding of the mathematics to the level necessary to understand, apply, and critically question the physical framework under study. Purely mathematical proofs and developments can be left for when they become necessary or when I decide to delve deeper into the foundations.

For example, it is not necessary, at this point, to delve deeper into the proof of the spectral theorem for Hermitian matrices, since its proof is predominantly mathematical and does not, by itself, add a new physical interpretation to the problem. On the other hand, it is worth deepening mathematical results that establish relationships directly relevant to the physical theory, such as proving that two operators that have the same complete basis of eigenvectors commute, since this relationship allows us to understand mathematically the compatibility between observables.

However, I will be sharing here a purely mathematical review, an almost verbatim reproduction of this material (\cite{DelftSpectralTheorem}). A more complete review can be found \href{https://github.com/jonasmaziero/algebra_linear}{aqui}.
...
Properties that are typically demonstrated in exercises:
\begin{itemize}
\item If $A$ is an $m\times n$ matrix and $B$ an $n\times m$ matrix, then $\text{Tr}\left(AB\right)=Tr\left(BA\right)$
\item If $A$ is an $n\times n$ matrix and $c$ a scalar, then $\text{Tr}\left(cA\right)=c\text{Tr}\left(A\right)$ and $\det\left(ca\right)=c^{n}\det\left(A\right)$
\end{itemize}
...
Properties that are typically demonstrated in exercises:
\begin{itemize}
\item Suppose that $A$ and $B$ are two $n\times n$ matrices and $P$ is an invertible $n\times n$ matrix such that $B=PAP^{-1}$, then $\text{Tr}\left(B\right)=\text{Tr}\left(A\right)$ and $\det B=\det A$, in addition, $A$ and $B$ share the same characteristic polynomial.
\end{itemize}
...
This means that $A-\lambda I$ is not invertible\footnote{If $B=(A-\lambda I)$ is invertible, it means that $B^{-1}B=I$ ,so $\boldsymbol{v}=B^{-1}0=0$ This is contradictory, since by definition $\boldsymbol{v}$ is nonzero. If it is not invertible, then $\text{det}\left(B\right)=0$.}; equivalently, its determinant vanishes. Thus, if $\lambda$ is an eigenvalue of $A$, we have
...
\begin{quotation}
\textbf{Definition}: An $n\times n$-matrix $A$ is diagonalisable if there exist a diagonal\footnote{A diagonal matrix is one that has elements only on the main diagonal $D_{ii}\neq0$, for any $j\neq0$, we have $D_{ij}=0$.} matrix $D$ and an invertible $n\times n$-matrix $P$ such that $A=PDP^{-1},$
\end{quotation}
...
\item Assuming $A=PDP^{-1}$ with $P$ invertible and $D$ a scalar matrix\footnote{A matrix is scalar when it is a multiple $c$ of the identity matrix $I$ , that is, $D=cI$ .}, then $A=D$.
\end{itemize}
...
\end{quotation}
We will not make the notion of completeness precise in these notes. Roughly speaking, it means that we do not have to worry too much about convergence of sequences or series in our Hilbert space, and there is a notion of infinite linear combinations\footnote{In short, we can think of a complete space as one in which a sequence of vectors cannot converge to a vector outside the space.}.
...
\item The Cauchy-Schwarz inequality states that for the vectors $\left|\alpha\right\rangle $ and $\left|\beta\right\rangle $, in a Hilbert space, we have $\left|\left\langle \alpha|\beta\right\rangle \right|^{2}\leq\left\langle \alpha|\alpha\right\rangle \left\langle \beta|\beta\right\rangle $

\item The triangle inequality states that for the vectors $\left|\alpha\right\rangle $ and $\left|\beta\right\rangle $, in a Hilbert space, we have $\left\Vert \left|\alpha\right\rangle +\left|\beta\right\rangle \right\Vert \leq\left\Vert \left|\alpha\right\rangle \right\Vert +\left\Vert \left|\beta\right\rangle \right\Vert $ 

\item If we consider $W$ a linear subspace of the Hilbert space $\mathcal{H},$ then the orthogonal complement\footnote{The orthogonal complement of $W$ is the set of vectors that are orthogonal to every vector in $W$. For example, if $W$ has as its basis the vectors $\boldsymbol{u}=\left(1,0,0\right)$ and $v=\left(0,1,0\right)$, then $W^{\perp}$ is the subspace corresponding to the $z$-axis.} $W^{\perp}$ is also a linear space.
\end{itemize}

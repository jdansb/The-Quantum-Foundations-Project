...
Como $B$ é Hermitiano, cada bloco $B_{j}$ também é uma matriz Hermitiana e, portanto, pode ser diagonalizado. Assim, podemos diagonalizar $B$ dentro de cada subespaço $\mathcal{H}_{j}$, obtendo uma base de autovetores de $B$ que também são autovetores de $A$. Assim provamos que se e semente se, dois operadores $A$ e $B$ comutam, então compartilham de uma base de autovetores em comum.
\begin{center}
\href{/exemplo-incompatibilidade}{Exemplo 1}.
\par\end{center}

Em resumo, quando temos um autovalor $a_{j}$ não degenerado, a situação é muito simples, pois nesse caso, o único autovetor de $A$, a menos de um fator multiplicativo, é automaticamente também um autovetor de $B$.

Quando $a_{j}$ é degenerado, seus autovetores formam um subespaço $\mathcal{H}_{j}$, correspondente ao autovalor $a_{j}$, no qual todo vetor não nulo também é um autovetor de $A$ associado a $a_{j}$. Como $A$ e $B$ comutam, a aplicação de $B$ sobre qualquer vetor de $\mathcal{H}_{j}$ resulta em outro vetor pertencente a $\mathcal{H}_{j}$. Assim, basta encontrar, dentro desse subespaço, os vetores que também sejam autovetores de $B$.

Essa possibilidade é garantida pelo fato de $B$ ser Hermitiano: sua atuação restrita a $\mathcal{H}_{j}$ pode ser representada por uma submatriz Hermitiana, que pode ser diagonalizada por uma base ortonormal de autovetores\footnote{O fato de uma matriz ser Hermitiana garante que podemos escolher, no espaço em que ela atua, uma base ortonormal formada por seus autovetores{[}\cite{DelftSpectralTheorem}, \href{https://interactivetextbooks.tudelft.nl/mqp-v2/operators.html}{nesta página}.{]}.}. Portanto, podemos escolher uma base ortonormal de $\mathcal{H}_{j}$ formada por autovetores de $B$. Como todos os vetores de $\mathcal{H}_{j}$ são também autovetores de $A$ associados a $a_{j}$, esses vetores são autovetores comuns de $A$ e $B$.

Repetindo esse procedimento para cada autoespaço de $A$, obtemos uma base ortonormal de $\mathcal{H}$ formada por autovetores comuns de $A$ e $B$.

Com a prova deste teorema, sabemos que se dois observáveis $A$ e $B$ de um sistema não comutam, eles não podem compartilhar todos os autovetores. Se, medindo o observável $A$, preparamos o sistema em um estado que não é autovetor do observável $B$, este vetor deve ser obrigatoriamente uma combinação linear dos autovetores de com duas ou mais amplitudes de probabilidade não nulas.
\begin{center}
\href{/exemplo-incompatibilidade}{Exemplo 2}.
\par\end{center}
...
\end{align}
com $\mathrm{Var}(X)=\mathrm{Var}(X)_{\xi}=\langle\xi|(X-\langle X\rangle I)^{2}|\xi\rangle=\langle(X-\langle X\rangle I)^{2}\rangle$ sendo a variância de $X$.
...
\langle\psi|\phi\rangle & =((A-\langle A\rangle I)|\xi\rangle,(B-\langle B\rangle I)|\xi\rangle)\\
 ...
 & =2^{-1}(\langle AB\rangle-\langle A\rangle\langle B\rangle+\langle BA\rangle-\langle A\rangle\langle B\rangle)+i(-i2^{-1}\langle[A,B]\rangle)
\end{align}
onde 
\begin{align}
 & [A,B]=AB-BA,\\
...
\begin{equation}
{\color{blue}{\mathrm{Var}(A)\mathrm{Var}(B)\ge(\mathrm{CovQ}(A,B))^{2}+2^{-2}|\langle[A,B]\rangle|^{2}}}\mathpunct{\color{blue},}
\end{equation}
...
\]
sendo a covariância entre os observáveis $X$ e $Y$. Vale observar que se $A$ e $B$ são observáveis compatíveis, i.e., se $[A,B]=0$, teremos que $\mathrm{CovQ}(A,B)=\mathrm{Cov}(A,B)$.
...
Ignorando-se o termo relacionado à covariância quântica na RIHRS e tomando a raiz quadrada da expressão que resta, obtemos a relação de incerteza de Robertson: 
\begin{equation}
\sigma_{A}\sigma_{B}\ge2^{-1}|\langle[A,B]\rangle|,
\end{equation}
...
Vale observar aqui que podemos usar a relação de incerteza de Robertson para obter uma relação que envolve a soma das variâncias: 
\begin{align}
 & (\sigma_{A}-\sigma_{B})^{2}\ge0\therefore\sigma^{2}_{A}+\sigma^{2}_{B}-2\sigma_{A}\sigma_{B}\ge0\\
 & Var(A)+Var(B)\ge2\sigma_{A}\sigma_{B}\ge|\langle[A,B]\rangle|.
\end{align}

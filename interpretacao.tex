...
\usepackage{babel}
\begin{document}
\title{What Is (Not) the Interpretation of Quantum Mechanics?}
...
A law of physics can be understood as a rule that describes, usually through a mathematical formulation, how certain physical systems behave\cite{Siddiqui2014}. Newton's second law, for example, can be expressed by the equation
...
Usando então o método de separação de variáveis, isto é, considerando que a solução pode ser separada em uma parte expacial $\Phi\left(x\right)$ e outra temporal $\Pi\left(t\right)$ então:
...
Diferentemente da MC, a MQ, em geral, não nos possibilita prever com certeza os resultados de medidas futuras. O que a MQ nos fornece são as frequências relativas (probabilidades) com que esses resultados são obtidos, que são calculadas via a regra de Born: 
...
Essa é a probabilidade de obtermos o valor ($o$) ao medirmos o observável ($O$) de um sistema que se encontra no estado $\left|\psi\right\rangle $ imediatamente antes da medida, sendo ($\left|o\right\rangle $) o autovetor de ($O$) correspondente ao autovalor ($o$).
...
Se temos dois operadores Hermitianos, escrito na sua representação matricial como:

\begin{align*}
A= & \sum^{d}_{j=1}a\left|a_{j}\right\rangle \left\langle a_{j}\right|\\
\end{align*}

...
\[
\Re\left(\left\langle \psi|\phi\right\rangle \right)^{2}=\text{CovQ}\left(A,B\right)^{2}=\left(\frac{\text{Cov\ensuremath{\left(A,B\right)}+Cov\ensuremath{\left(BA\right)}}}{2}\right)^{2}
\]
...
O segundo ponto é que é incorreto interpretar a relação de incerteza como afirmando que uma partícula \textquotedbl não tem posição e momento definidos\textquotedbl{} ou que \textquotedbl não podemos medir posição e momento na mesma preparação\textquotedbl . A primeira afirmação é uma interpretação possível, mas não é o conteúdo matemático da relação, enquanto a segunda é simplesmente incorreta. A relação de incerteza não diz que é impossível obter resultados de posição e momento na mesma preparação; ela estabelece uma relação entre as variâncias dos resultados dessas medidas.

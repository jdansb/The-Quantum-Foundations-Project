...
Então nós temos que se os operadores compartilham do mesmo conjunto de autovetores eles comutam. Podemos agora provar o contrário:
...
Agora se $a_{j}$ é degenerado, isto é, existem dois ou mais autovetores linearmente independentes correspondentes ao mesmo autovalor $a_{j}$, então existe um subespaço de $\mathcal{H}$ gerado pelos autovetores de $A$ correspondentes a $a_{j}$. É um pouco mais complexo, mas é a mesma ideia aplicada a um subespaço de dimensão maior que 1. Para entender isso, vamos olhar especificamente para uma outra fonte {[}\cite{MIT804_CommonEigenbases}{]}.

Vamos chamar esse subespaço de $\mathcal{H}_{j}$ e sendo $\left\{ \left|a^{k}_{j}\right\rangle \right\} ^{d_{j}}_{k=1}$ uma base qualquer de $\mathcal{H}_{j}$, entao $A\left|a^{k}_{j}\right\rangle =a_{j}\left|a^{k}_{j}\right\rangle $. Agora ainda temos que $B\left|a_{j}\right\rangle \in\mathcal{H}_{j}$, isto é, $B$ transforma vetores de $\mathcal{H}_{j}$ em vetores que também pertencem a $\mathcal{H}_{j}$. Portanto cada $B\left|a^{k}_{j}\right\rangle $ é uma combinação linear dos vetores da base de $\mathcal{H}_{j}$

$B\left|a^{k}_{j}\right\rangle =\sum^{d_{j}}_{l=1}b_{l}\left|a^{l}_{j}\right\rangle $.

Como $B$ é um operador Hermitiano\footnote{Ser diagonalizável por uma base ortonormal é uma propriedade dos operadores Hermitianos em dimensão finita.}, existe então uma base de $\mathcal{H}_{j}$ que o diagonaliza. Assumindo que a base é $\left\{ \left|a^{k}_{j}\right\rangle \right\} $, temos $B\left|a^{k}_{j}\right\rangle =b_{j}\left|a^{k}_{j}\right\rangle $. Ou seja, se os operadores $A$ e $B$ comutam, então compartilham da mesma base de autovetores.
